...
Like the graphics pipeline, text rendering can be split into three categories: graphics level, engine level,  
and user level.  
...
At a engine level, we take the font and create a texture and we handle how characters should be spaced  
out together. Items like word wrapping, text coloring, or other more detailed options should be handled here.  
At a graphics level, we take a series of boxes from the words and turn them into a single render object.  
...
vertices, since we can compute those. **EDIT**: This doesn't work given the below specifications? Need to rethink.
