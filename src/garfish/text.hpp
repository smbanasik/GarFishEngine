...
#include <garfish_user/types.hpp>
...
// TextBox - a box of text, use as few as possible. Contains the text buffer,
// and any info needed to render a piece of text. Font - contains any
// information needed to create and use a font. Character - contains any
// information needed to construct a character. How does a user interact with
// this? The user should be able to
...
public:
  // TODO: better options for modification. Appending or recording what's
  // changed could save work
...
public:
  ...
  void
  add_font_from_file(const std::string &font_name, const std::string &font_path,
                     unsigned int pix_size =
                         48); // TODO: turn this into point instead of pixel
  const Font *get_font(const std::string &font_name) {
    return &fonts.at(font_name);
  };

  TextBox *create_textbox(const std::string &text_box_name,
                          const std::string &text_buffer = "");
  TextBox *get_textbox(const std::string &text_box_name) {
    return &text_boxes.at(text_box_name);
  };
...
private:
  ...
  TextBox initialize_textbox(const std::string &text_box_name,
                             const std::string &text_buffer);
