...
public:
    ...
    * @author Spencer Banasik
    */
    void set_key_mapping(int key, std::function<void(wi::WInputContext* context, Key* key)>&& function);

    /**
    * @brief Allows the user to add their own functionality to the key action callback, which will always be called.
    * @author Spencer Banasik
    */
    void set_callback_keyboard_key(std::function<void(WInputContext* wi_context, Key* key)>&& function) { callback_keyboard_key = std::move(function); };
    
    /**
    * @brief Allows the user to add their own functionality to the character callback.
    * @author Spencer Banasik
    */
    void set_callback_keyboard_char(std::function<void(WInputContext* wi_context)>&& function) { callback_keyboard_char = std::move(function); };
...
private:
    ...
    */
    ...
    std::function<void(WInputContext* wi_context)> callback_keyboard_char;
    std::function<void(WInputContext* wi_context, Key* key)> callback_keyboard_key;
    GLFWwindow* window_handle = nullptr;
    std::unordered_map <int, std::function<void(wi::WInputContext* context, Key* key)>> key_map;
